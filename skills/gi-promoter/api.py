"""Programmatic entry point for gi-promoter.
...
private helper functions inside gi_promoter.py.
"""
...
_SCRIPT = _SKILL_DIR / "gi_promoter.py"
...
def run(input_path: str, output_dir: str = "/tmp/gi-promoter") -> dict:
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            f"gi-promoter exited with code {completed.returncode}: {completed.stderr.strip()}"
        )
    result_json = out / "result.json"
    if not result_json.exists():
        raise RuntimeError(f"gi-promoter produced no result.json at {result_json}")
