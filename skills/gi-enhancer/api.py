"""Programmatic entry point for gi-enhancer.
...
private helper functions inside gi_enhancer.py.
"""
...
_SCRIPT = _SKILL_DIR / "gi_enhancer.py"
...
def run(input_path: str, output_dir: str = "/tmp/gi-enhancer") -> dict:
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            f"gi-enhancer exited with code {completed.returncode}: {completed.stderr.strip()}"
        )
    result_json = out / "result.json"
    if not result_json.exists():
        raise RuntimeError(f"gi-enhancer produced no result.json at {result_json}")
