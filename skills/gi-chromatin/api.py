"""Programmatic entry point for gi-chromatin.
...
private helper functions inside gi_chromatin.py.
"""
...
_SCRIPT = _SKILL_DIR / "gi_chromatin.py"
...
def run(input_path: str, output_dir: str = "/tmp/gi-chromatin") -> dict:
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            f"gi-chromatin exited with code {completed.returncode}: {completed.stderr.strip()}"
        )
    result_json = out / "result.json"
    if not result_json.exists():
        raise RuntimeError(f"gi-chromatin produced no result.json at {result_json}")
